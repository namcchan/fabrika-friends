...
def convert_to_unix(time_stamp: str):
    dt_obj = datetime.strptime(
        time_stamp, '%Y-%m-%dT%H:%M:%S.%fZ').replace(tzinfo=timezone.utc)
...
class Tapper:
    ...
    async def get_tg_web_data(self, proxy: str | None) -> str:
        try:
            ...
            logger.error(
                f"{self.session_name} | Ref link invaild please check again !")
            sys.exit()
        ...
        try:
            ...
            tg_web_data = unquote(string=auth_url.split('tgWebAppData=')[
    ...
    def skip_onboarding(self, session: requests.Session):
        try:
            ...
            requests.options(
                "https://api.ffabrika.com/api/v1/profile", headers=head1)
            payload = {
                "isOnboarded": True
            }
            res = session.patch(
                "https://api.ffabrika.com/api/v1/profile", json=payload)
    ...
    async def get_user_info(self, session: requests.Session):
        ...
        if res.status_code == 200:
            ...
            if check_new_user is False:
                ...
                logger.info(
                    f"{self.session_name} | Attempt to create new account...")
                await asyncio.sleep(randint(2, 5))
                attempt = 3
                while True:
                    self.skip_onboarding(session)
                    res = session.get(
                        "https://api.ffabrika.com/api/v1/profile")
                    user_data = res.json()
                    check_new_user = user_data['data']['isOnboarded']
            ...
            print(res.text)
            logger.warning(
                f"{self.session_name} | Can't get user info: {res.status_code}")

    def join_squad(self, session: requests.Session):
        sid = settings.SQUAD_ID
        res = session.post(
            f"https://api.ffabrika.com/api/v1/squads/joining/{sid}")
        if res.status_code == 204 or res.status_code == 401:
            return res.status_code
        else:
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to join squad: {res.status_code}</yellow>")

    def do_task(self, session: requests.Session, task_id, task_des):
        res = session.post(
            f"https://api.ffabrika.com/api/v1/tasks/completion/{task_id}")
        if res.status_code == 201:
            ...
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to do task {task_des}: {res.status_code}</yellow>")

    def need_to_work(self, session: requests.Session):
        res = session.get(
            f"https://api.ffabrika.com/api/v1/factories/{self.factory_id}/workers?page=1")
        if res.status_code == 200:
    ...
    def login(self, session: requests.Session):
        ...
        session.options(
            "https://api.ffabrika.com/api/v1/auth/login-telegram", headers=head1)
        ...
        response = session.post(
            "https://api.ffabrika.com/api/v1/auth/login-telegram", json=payload)
    ...
    def fetch_tasks(self, session: requests.Session):
        ...
        if res.status_code == 200:
            ...
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to fetch tasks: {res.status_code}</yellow>")
            return []
    ...
    def tap(self, session: requests.Session, tap_count: int):
        ...
        res = session.post(
            "https://api.ffabrika.com/api/v1/scores", json=payload)
        if res.status_code == 201:
            data = res.json()['data']
            logger.success(
            ...
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to tap: {res.status_code}</yellow>")

    def boost_energy(self, session: requests.Session):
        res = session.post("https://api.ffabrika.com/api/v1/energies/recovery")
        if res.status_code == 201:
            logger.success(
                f"{self.session_name} | <green>Successfully use energy boost !</green>")
            data = res.json()
            ...
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to boost energy: {res.status_code}</yellow>")
        return False
    ...
    def get_factory_info(self, session: requests.Session):
        res = session.get(
            f"https://api.ffabrika.com/api/v1/factories/{self.factory_id}")
        if res.status_code == 200:
            ...
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to get factory info: {res.status_code}</yellow>")
            return None

    def purchase(self, session: requests.Session, worker_id):
        res = session.post(
            f"https://api.ffabrika.com/api/v1/market/workers/{worker_id}/purchase")
    ...
    def get_score(self, session):
        ...
        if res.status_code == 200:
            data_ = res.json()
            logger.info(
                f"{self.session_name} | Balance: <cyan>{data_['data']['balance']}</cyan>")
    ...
    def get_user_info2(self, session: requests.Session):
        ...
        if res.status_code == 200:
            data = res.json()['data']
            logger.info(
                f"{self.session_name} | Balance: <cyan>{data['score']['balance']}</cyan>")

    def get_workers_data(self, session: requests.Session):
        res = session.get(
            "https://api.ffabrika.com/api/v1/market/workers?page=1&limit=20")
        if res.status_code == 200:
    ...
    def collect_reward(self, session: requests.Session, value):
        res = session.post(
            "https://api.ffabrika.com/api/v1/factories/my/rewards/collection")
        if res.status_code == 204:
            logger.success(
                f"{self.session_name} | <green>Successfully claimed <cyan>{value}</cyan></green>")
    ...
    async def send_worker_to_work(self, session: requests.Session):
        ...
        res = session.post(
            f"https://api.ffabrika.com/api/v1/factories/my/workers/tasks/assignment", json=payload)
        if res.status_code == 204:
            logger.success(
                f"{self.session_name} | <green>Successfully sent all worker to work!</green>")
        elif res.status_code == 401:
            return self.refresh_token(session)
        else:
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to send worker to work: {res.status_code}</yellow>")

    def refresh_token(self, session: requests.Session):
        headers['Cookie'] = f"ref_uid={self.refresh_token}"
        res = session.post(
            "https://api.ffabrika.com/api/v1/auth/refresh", json={})
        if res.status_code == 201:
            data_ = res.json()['data']
            ...
            logger.success(
                f"{self.session_name} | <green>Refresh token successfully !</green>")
        else:
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to refresh token:</yellow> {res.status_code}")
    ...
    def buy_workplace(self, session: requests.Session):
        ...
        res = session.post(
            "https://api.ffabrika.com/api/v1/tools/market/5/purchase", json=payload)
        if res.status_code == 204:
            logger.success(
                f"{self.session_name} | <green>Successfully bought a new workplace!</green>")
            self.balance -= 60000
        elif res.status_code == 401:
            return self.refresh_token(session)
        else:
            print(res.text)
            logger.warning(
                f"{self.session_name} | <yellow>Failed to buy workplace:</yellow> {res.status_code}")
    ...
    async def run(self, proxy: str | None) -> None:
        ...
        if proxy:
            ...
            if proxy_check:
                ...
                logger.info(
                    f"{self.session_name} | bind with proxy ip: {proxy}")
            ...
            try:
                ...
                if can_run:
                    ...
                    if self.login(session):
                        ...
                        self.last_boost_used = self.user_data['energy'][
                            'lastRecoveryAt'] if self.user_data['energy']['lastRecoveryAt'] else 0
    ...
    Daily Reward:
        ...
        --Claimed: <red>{self.user_data['dailyReward']['isRewarded']}</red>
                        """
                        logger.info(
                            f"{self.session_name} | User stats: \n{user_stats}")

                        if self.user_data['dailyReward']['isRewarded'] is False:
                            res = session.post(
                                "https://api.ffabrika.com/api/v1/daily-rewards/receiving")
                            ...
                            if res.status_code == 401:
                                ...
                                res = session.get(
                                    "https://api.ffabrika.com/api/v1/scores")
                                if res.status_code == 200:
                                    logger.success(
                        ...
                        if squad == "No squad":
                            ...
                            if res_code == 204:
                                res = session.get(
                                    f"https://api.ffabrika.com/api/v1/squads/{settings.SQUAD_ID}")
                                if res.status_code == 200:
                                    logger.success(
                            ...
                            if task_list is not None:
                                for task in task_list:
                                    if task['isCompleted'] is False:
                                        self.do_task(
                                            session, task['id'], task['description'])
                    ...
                    if settings.AUTO_MANAGE_FACTORY:
                        ...
                        if factory_info:
                            ...
                            if factory_info['rewardCount'] > 0:
                                self.collect_reward(
                                    session, factory_info['rewardCount'])
                    ...
                    if settings.AUTO_TAP:
                        ...
                        while i > 0:
                            ...
                            if self.enery > settings.SLEEP_BY_MIN_ENERGY:
                                ...
                                self.tap(session, tap_count)
                                sleep_ = random.uniform(
                                    settings.SLEEP_BETWEEN_TAPS[0], settings.SLEEP_BETWEEN_TAPS[1])
                                logger.info(
                                    f"{self.session_name} | sleep <red>{sleep_}s</red>")
                                ...
                                if settings.AUTO_BOOST:
                                    if self.energy_boost > 1:
                                        if self.check_time(self.last_boost_used):
                                            ...
                                            logger.info(
                                                f"{self.session_name} | Not time to use boost!")
                                            break
                                    else:
                                        logger.info(
                                            f"{self.session_name} | No energy boost left...")
                                        break
                                else:
                                    break
...
async def run_tapper1(tg_clients: list[Client], proxies):
    ...
    while True:
        for tg_client in tg_clients:
            ...
            sleep_ = randint(
                settings.DELAY_EACH_ACCOUNT[0], settings.DELAY_EACH_ACCOUNT[1])
